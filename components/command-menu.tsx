...
import { SiX, SiLinkedin, SiGithub, SiLeetcode } from "react-icons/si"
        ...
        const down = (e: KeyboardEvent) => {
            ...
            if (e.shiftKey) {
                ...
                // Links
                    ...
                    e.preventDefault()
                    runCommand(() => openExternal(site.socials.x))
                } else if (key === 'l') {
                    e.preventDefault()
                    runCommand(() => openExternal(site.socials.linkedin))
                } else if (key === 'g') {
                    e.preventDefault()
                    runCommand(() => openExternal(site.socials.github))
                } else if (key === 'o') {
                    e.preventDefault()
                    runCommand(() => openExternal(site.socials.leetcode))
                } else if (key === 'e') {
                    e.preventDefault()
        ...
        <>
            ...
            <CommandDialog open={open} onOpenChange={setOpen} className="font-custom2">
                ...
                <div className="flex items-center gap-4 p-4">
                    ...
                    <div>
                        <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">{site.name}</h3>
                        <p className="text-xs text-neutral-500 dark:text-neutral-400">{site.role} @ {site.company}</p>
                    </div>
                </div>
                ...
                <CommandList className="font-custom2 p-2">
                    ...
                    <CommandGroup heading="Links">
                        <CommandItem onSelect={() => runCommand(() => openExternal(site.socials.github))} className="rounded-lg py-3">
                            <SiGithub className="mr-2 h-4 w-4 text-neutral-500" />
                            <span>GitHub — @{site.githubAccounts[0].username}</span>
                        ...
                        <CommandItem onSelect={() => runCommand(() => openExternal(site.socials.githubWork))} className="rounded-lg py-3">
                            <SiGithub className="mr-2 h-4 w-4 text-neutral-500" />
                            <span>GitHub (work) — @{site.githubAccounts[1].username}</span>
                        </CommandItem>
                        <CommandItem onSelect={() => runCommand(() => openExternal(site.socials.linkedin))} className="rounded-lg py-3">
                        ...
                        <CommandItem onSelect={() => runCommand(() => openExternal(site.socials.leetcode))} className="rounded-lg py-3">
                            <SiLeetcode className="mr-2 h-4 w-4 text-neutral-500" />
                            <span>LeetCode Profile</span>
                            <CommandShortcut className="font-mono text-[10px] bg-neutral-100 dark:bg-neutral-800 px-1.5 py-0.5 rounded border border-neutral-200 dark:border-neutral-700">shift + O</CommandShortcut>
                        </CommandItem>
                        <CommandItem onSelect={() => runCommand(() => openExternal(site.socials.x))} className="rounded-lg py-3">
